...
| `collect_post_sparse_gap_run_segments` | `apps/api/app/transcription.py` | In a long gap with an early onset and a later dense run, inject extra segments for the late run. | Preserve long-gap lower/mixed runs that otherwise collapse. | `kalimba-17-c-bwv147-lower-mixed-roll-01`; `kalimba-17-c-bwv147-lower-context-roll-01`; indirectly relevant to `kalimba-17-c-bwv147-lower-f4-mixed-run-01`; tested by `test_bwv147_lower_mixed_roll_recovers_opening_mixed_dyad_and_long_gap_run` | This is a segmentation workaround for one phrase geometry: long gap plus later lower-run cluster. | A more general onset-to-segment model that can represent separated subphrases without ad hoc gap-run injection. |
...
| `kalimba-17-c-bwv147-lower-context-roll-01` | `completed` | `lower-mixed-roll-extension`, `collect_post_sparse_gap_run_segments` | Context-preserved lower-roll phrase. |
| `kalimba-17-c-bwv147-lower-mixed-roll-01` | `pending` | `lower-mixed-roll-extension`, `collect_post_sparse_gap_run_segments` | Current blocker is mainly scoped-evaluation / scoping debt, not recognizer debt. |
