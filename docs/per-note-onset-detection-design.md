...
### E83 型: fast mute-dip (解決済み)

E83 G4 は genuine re-attack（耳確認済み）。当初「mute-dip なし」と判定されていたが、
fine-grained peak tracking (5ms step) で **~20ms の fast mute-dip** を確認:

```
47.005s  energy=24.8  (安定した残響)
47.020s  energy= 5.0  (急落)
47.025s  energy= 1.3  (最深点 — 100x以上の drop)
47.050s  energy=26.2  (完全回復)
```

既存の 50ms FFT 窓がこの高速 dip を平滑化 (dip_ratio=0.14、閾値 0.10 を超過) していた。
30ms 窓フォールバックの追加で検出可能に (dip_ratio=0.035):
- Initial primary = D5 (残響が spectral rank 1位)
- G4 の mute-dip が 30ms 窓で検出 → residual-forward-scan で G4 に promotion
- 修正コミット: `e599d4b`

### Note-state model との関連

現在の mute-dip 検出は state-less（各 onset で独立判定）。研究マッピング §4 で提案されている
`OFF → ATTACK → BODY → LATE_DECAY (→ RESONANCE_ONLY)` の note-state 遷移モデルを
導入すれば、E83 型の問題をより統一的に扱える:

- note が **BODY** 状態で energy が一瞬途切れた → **ATTACK** に遷移（mute-dip 検出と等価）
- note が **LATE_DECAY** 状態で energy が上昇 → **ATTACK** に遷移（onset gain ベース）
- note が **RESONANCE_ONLY** 状態 → onset がない限り note-on を許可しない（共鳴 FP 抑制）

現行の局所的修正（50ms → 30ms フォールバック）で E83 は解決したが、今後の per-note 判定が
増えるにつれ state model による統一的な管理が自然になる。Note dataclass (#142) に state
フィールドを追加する形で段階的に導入可能。

関連: 研究マッピング §2 (onset gate 強化)、§4 (note-state 明示化)、#141 umbrella
...
- ~~**E83 型 (mute-dip なし re-attack)**~~: 解決済み。実際には fast mute-dip (~20ms) が存在していた。30ms 窓フォールバックで検出可能
