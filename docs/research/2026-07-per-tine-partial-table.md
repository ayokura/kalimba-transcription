...
### 2. 安定に観測される in-band 成分は ×2.8-2.9 (物理モード名は未確定 — fugu-ultra 監査 2026-07-05 で訂正)

17-C tester (n=138 イベント): E4 ×2.913 (sup 1.0) / G4 ×2.883 (sup 0.96) / C5 ×2.91 (sup 0.95) / E5 ×2.821 (sup 0.82) / A4 ×3.003 (sup 1.0, n=4) / G5 ×2.652 (sup 1.0)。これらの周波数は 17-C の tine 基音の間に落ちる (例: G4×2.883=1130Hz は C6 と D6 の間) ため残響では説明できず、打鍵に付随する実成分と判断できる。比が tine ごとに固有 (×2.65-3.00) である点は per-tine 識別特徴としての利用可能性を維持する。

**訂正 (fugu-ultra 監査)**: 当初「第 2 モード」「梁理論と整合」とラベルしたが、理想 clamped-free 梁の f2/f1 は 6.267 であり ×2.8-2.9 はどの片持ち梁モードにも対応しない — **物理モード名は未確定** (捩れモード・bridge 系・筐体系等の可能性)。梁理論の第 1 overtone は探索帯域 (×1.2-4.4) の外にあり、拡張プローブ ([`high-partial-probe.json`](high-partial-probe.json)) で **×5.6-6.6 に support ほぼ 100% で実在確認** (tester G4 ×5.732 28/28、C5 ×5.838 13/14、author C4 ×6.548 5/5)。また「×2.0 partial は存在しない」という表現も table 自身 (A5 ×2.002 / B5 ×2.107) と矛盾するため撤回する。

### 2b. 本 table の正確な位置づけ: in-band collision table (×1.2-4.4)

PARTIAL_BAND=[1.2, 4.4] は「**他 tine 基音と衝突しうる帯域**」を意図した設計であり、tine の全 partial 構造の記述ではない。現 17-C tuning では最高 tine が E6 (1319Hz) のため、×5.6-6.6 成分 (G4×5.73≈2246Hz 等) は全 tine 基音より上 = 衝突域外であり、explaining-away 機構には不要。**ただしレンジの広い楽器 (34 鍵の上位等) では ×5.8 帯が衝突域に入りうる** — その時点で band 拡張が必要 (fugu-ultra 指摘の限定条件)。
