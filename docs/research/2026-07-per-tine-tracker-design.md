...
### 位相勾配による partial / 実打鍵 / 共鳴の識別 (ユーザー発案 2026-07-05、アイディア段階)

- **partial vs 実打鍵**: victim 帯域に現れた成分の振動周波数を位相勾配で読む。実打鍵なら victim tine の固有周波数 (per-tine 実測 detune)、partial なら厳密に striker 基音 × 実測比 (例: C4×3.02 は G5 に対し +15c ≈ 100ms で 4.3 rad — 楽に分解可能。実測 table 最小の +4c でも ~1.9 rad)。立ち上がり同時性 + striker との包絡相関、実測 bleed 振幅 prior と合わせて 3 重の識別子になる。実打鍵 + partial の同時発生 (和音) は 2 成分混合だが、ビート (振幅変調) がむしろ「2 成分の証拠」になる。ROC の FN 主因 (倍音一致メロディ音の抑圧) を狙い撃つ識別子
- **共鳴**: victim が自分の固有周波数で実振動するため周波数識別は無効。残る識別子は (a) 位相リセットの有無 — 打鍵は不連続、共鳴は滑らかな build-up。**「共鳴を新規打鍵と誤認しない」FP 抑制方向には位相が有効**、(b) driven (driver と包絡相関) vs impulse response のエンベロープ形状、(c) 実測 coupling 振幅 prior (fresh 汚染度と同手法で実測可能)。ただし機械結合の impulse 伝達 (34L-C K13↔K30 型) は物理的に弱打鍵と同一励起のため位相でも原理的にほぼ区別不能 — 振幅 prior と文脈のみが残る (ユーザーの直感と一致)
