...
K6 について: スプリント再定義 (人間 GO 区切り) により時間 box 性が弱まったとの監査指摘があった。**「S5 実装開始から 3 巡以内、または 30 暦日以内のいずれか早い方」への読み替えを提案する** — ただしこれは kill 条件の変更に当たるため、ユーザー承認 + 本追記をもって変更手続きの記録とする (承認まで旧定義が有効)。
